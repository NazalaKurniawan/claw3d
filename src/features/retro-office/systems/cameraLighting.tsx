...
const DAY_NIGHT_POSITIONS = [0, 0.15, 0.35, 0.5, 0.65, 0.85];
...
export function DayNightCycle({
  ...
  useFrame((_, delta) => {
    if (externalTimeRef) {
      ...
      const d = new Date();
      // Shift by 6 hours so 0.0 is 6 AM
      let t = ((d.getHours() + d.getMinutes() / 60 + d.getSeconds() / 3600) - 6) / 24;
      if (t < 0) t += 1;
      timeRef.current = t;
    }
    ...
    if (scene && "environmentIntensity" in scene) {
      scene.environmentIntensity = lerp(
        keyframeA.envIntensity,
        keyframeB.envIntensity,
        localT,
      );
    }
  });
