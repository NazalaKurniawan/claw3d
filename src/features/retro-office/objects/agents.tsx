...
export const AgentModel = memo(function AgentModel({
  ...
  useFrame(() => {
    ...
    const isIdleStanding = agent.state === "standing" && !agent.pingPongUntil;
    const idlePhase = agent.frame * 0.012 + (agent.phaseOffset ?? 0) * 3.7;
    // Periodic head look: slow sine wave that makes the agent glance left/right
    const idleHeadSway = isIdleStanding
      ? Math.sin(idlePhase * 0.8) * 0.12
      : 0;
    // Tiny hop every ~8 seconds (frame ~480 at 60fps): a sharp sine bump
    const hopCycle = (agent.frame + (agent.phaseOffset ?? 0) * 137) % 480;
    const idleHop = isIdleStanding && hopCycle < 12
      ? Math.sin((hopCycle / 12) * Math.PI) * 0.035
      : 0;
    // Subtle body tilt during idle
    if (isIdleStanding) {
      groupRef.current.rotation.z = Math.sin(idlePhase * 0.5) * 0.02;
    }
    // Apply head sway to facing rotation
    groupRef.current.rotation.y += idleHeadSway;
    ...
    const breathe =
      agent.state === "standing" || isWorkout || agent.pingPongUntil
        ...
        : 0;
    groupRef.current.position.y = bounce + breathe + idleHop;
    ...
    if (leftArmRef.current) {
      ...
      } else if (isIdleStanding) {
        // Idle arm fidget: subtle arm sway
        leftArmRef.current.rotation.x = Math.sin(idlePhase * 1.1) * 0.06;
        leftArmRef.current.rotation.z = -0.04 + Math.sin(idlePhase * 0.7) * 0.03;
      }
    }
    if (rightArmRef.current) {
      ...
      if (isJanitor && janitorTool !== "broom") {
        ...
        // Idle arm fidget: subtle opposite-phase sway
        rightArmRef.current.rotation.x = Math.sin(idlePhase * 1.1 + Math.PI) * 0.06;
        rightArmRef.current.rotation.z = 0.04 - Math.sin(idlePhase * 0.7) * 0.03;
      }
    }
    ...
    const ambientBubbleVisible =
      (!suppressSpeechBubble && (isError || working)) ||
      (!isAway &&
